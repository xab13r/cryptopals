...
The attack on secret-prefix SHA1 relies on the fact that you can take the ouput 
of SHA-1 and use it as a new starting point for SHA-1, 
...
the SHA-1 hash in this fashion will appear to have been hashed with the secret key.
...
message = b"comment1=cooking%20MCs;userdata=foo;comment2=%20like%20a%20pound%20of%20bacon"
target = b"comment1=cooking%20MCs;userdata=foo;comment2=%20like%20a%20pound%20of%20bacon;admin=true"
...
def SHA1_Padding(message):
    ...
    padding += b"\x80"
    # Append 0 <= k < 512 bits '0', so that the resulting message length (in bytes) is congruent to 56 (mod 64)
